...
document.querySelectorAll('.entrada').forEach(entrada => {
    ...
    const iframe = entrada.querySelector('iframe');
    const baseWidth = 30; // Base size in percentage
    const aspectRatio = 9 / 16; // Relación de aspecto del video (alto/ancho)

    // Calcula el ancho del iframe
    const newWidth = baseWidth + wordCount * 0.1;
    iframe.style.width = newWidth + '%'; // Ajusta el ancho del iframe

    // Calcula el alto del iframe en función del ancho y la relación de aspecto
    const newHeight = (newWidth * aspectRatio) / baseWidth * 100;
    iframe.style.height = newHeight + '%'; // Ajusta el alto del iframe
...
document.querySelectorAll('.entradaDestacada').forEach(entrada => {
    const paragraph = entrada.querySelector('p');
    let text = paragraph.innerText;
    const words = text.split(' ');
    
    if (words.length > 70) {
        text = words.slice(0, 70).join(' ') + '...'; // Limita a 100 palabras
    }
    ...
    const wordCount = text.split(' ').length; // Cuenta las palabras después de truncar
    const iframe = entrada.querySelector('iframe');
    const baseWidth = 40; // Base size in percentage
    const aspectRatio = 9 / 16; // Relación de aspecto del video (alto/ancho)
